...
> 基于大模型的智能云盘与文档管理平台 — AI 驱动的文件管理、语义搜索与内容生成
...
项目采用**双服务架构**设计：Java 21 + Spring Boot 3.2 构建后端业务服务，Python FastAPI 提供 AI 推理层；前端基于 React 19 + TypeScript + Vite。已完成 Docker 容器化并支持一键部署至阿里云 ECS。

### 为什么做这个项目

传统云盘仅提供存储和预览功能，用户需要手动整理文件、查找信息、生成内容——效率低下。NebulaMind 的目标是以 AI 重构文件管理体验：让云盘从"存储工具"进化为"知识助手"。
...
| 智能分类 | AI 自动分类与标签生成 |
| 多存储后端 | 支持 MinIO/S3 对象存储、本地存储、联通云盘（预留） |
...
| 对象存储 | MinIO (S3) vs 本地磁盘 | 本地开发与生产统一走 S3 协议，MinIO 与联通云盘等云存储兼容 |
...
| V2.0 | 存储抽象层重构、联通云盘对接、WebDAV/S3 兼容 | ✅ 完成 |
